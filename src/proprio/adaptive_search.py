"""Bounded search and qualification for adaptive instrument skills."""
...
def run_archive_search(
    ...
    *,
    ...
    for round_index in range(1, repair_rounds + 1):
        if any(
            entry.suite.verdict == "ADMIT" and entry.promotion_eligible for entry in entries
        ):
            break
    ...
    qualified = [
        entry
        for entry in entries
        if entry.suite.verdict == "ADMIT" and entry.promotion_eligible
    ]
