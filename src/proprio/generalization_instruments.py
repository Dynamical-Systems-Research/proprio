"""External simulator runtimes for the v0.3 generalization study.
...
def _north_paths() -> tuple[Path, Path]:
    root = _external_root(
        "PROPRIO_NORTH_ROOT", Path("/tmp/proprio-candidates/North-Cytation")
    )
...
class HelaoGamryController(ExternalController):
    ...
    def potential_cycle(
        ...
        applied_scale = (
            ...
            if self.potential_cycles == 0
            else self.subsequent_potential_scale
        )
...
class ClsLabLightController(ExternalController):
    ...
    def __init__(self, scenario: SimulationScenario, condition: Mapping[str, float]) -> None:
        ...
        self.maximum_signal = float(
            self.condition.get("maximum_signal", default_maximum_signal)
        )
...
GENERALIZATION_INSTRUMENTS: dict[str, ExternalInstrumentDefinition] = {
    "north-pipette-calibration": ExternalInstrumentDefinition(
        ...
        source_path=ROOT / "sources/generalization/north-pipette-calibration/source.md",
    ...
    "helao-gamry-cv": ExternalInstrumentDefinition(
        ...
        source_path=ROOT / "sources/generalization/helao-gamry-cv/source.md",
        allowed_methods=frozenset(
            {
                "reset",
                "connect",
    ...
    "clslab-light-spectrometer": ExternalInstrumentDefinition(
        ...
        source_path=ROOT / "sources/generalization/clslab-light-spectrometer/source.md",
        allowed_methods=frozenset(
...
def load_generalization_source(instrument_id: str) -> tuple[str, str]:
...
def external_simulator_identity(instrument_id: str) -> dict[str, Any]:
    ...
    payload["verdict"] = (
        "PASS"
        if revision == definition.upstream_revision and not tracked_changes
        else "FAIL"
    )
    return payload
...
def evaluate_generalization_skill(
    instrument_id: str,
    skill_py: str,
    *,
...
def run_generalization_preflight(instrument_id: str) -> FixturePreflightReport:
    definition = GENERALIZATION_INSTRUMENTS[instrument_id]
    ...
    return run_fixture_preflight(cases, evaluator=evaluate_generalization_skill)
...
def run_generalization_metrology(
    instrument_id: str,
    *,
    ...
    for index in range(cases_per_class):
        scenario = SimulationScenario.NOMINAL
        gate = evaluate_generalization_skill(
            instrument_id,
    ...
    return {
        "schema_version": "proprio.generalization_metrology.v0.3",
        "instrument_id": instrument_id,
