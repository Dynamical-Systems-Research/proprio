...
def test_generalization_panel_has_three_distinct_external_instruments() -> None:
    assert set(GENERALIZATION_INSTRUMENTS) == {
        "north-pipette-calibration",
        "helao-gamry-cv",
        "clslab-light-spectrometer",
    }
    assert len({definition.family for definition in GENERALIZATION_INSTRUMENTS.values()}) == 3
    for instrument_id in GENERALIZATION_INSTRUMENTS:
        source, digest = load_generalization_source(instrument_id)
...
def test_external_preflight_and_metrology_evidence_passes_fail_closed_bars() -> None:
    evidence_root = ROOT / "artifacts/evidence/generalization-v0.3"
    for instrument_id in GENERALIZATION_INSTRUMENTS:
