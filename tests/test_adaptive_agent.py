...
from proprio.adaptive_agent import (
...
def _candidate() -> CandidatePackage:
    _, source_hash = _source_loader("adaptive-fixture")
...
def test_adaptive_prompt_declares_bounded_feedback_driven_execution() -> None:
    normalized = " ".join(ADAPTIVE_SKILL_ENGINEER_SYSTEM_PROMPT.split())
...
def test_agent_repairs_only_after_debug_evidence_and_replay() -> None:
    ...
    agent = AdaptiveInstrumentAgent(
        ...
        families={"adaptive-fixture": "fixture"},
    )
...
def test_retryable_transport_failure_is_recorded_without_spending_model_turn(
    ...
    monkeypatch.setattr("proprio.instrument_agent.time.sleep", lambda _: None)
    condition = DebugCondition(
        condition_id="noise-repeat",
        scenario=SimulationScenario.NOMINAL,
        repetitions=3,
