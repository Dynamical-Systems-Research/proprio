def run(controller):
    ...
    if status == 0:
        ...
        if row_state["chip_id"] != initial_state["chip_id"] or row_state["state_nonce"] != initial_state["state_nonce"] or row_state["corner_found"] != initial_state["corner_found"]:
            status = 1

    for iy in range(2):
        for ix in range(4):
            if status == 0:
                arrival = controller.move_to_tile(ix, iy + 1)
            if status == 0 and not arrival:
                status = 4
            if status == 0 and arrival:
                focus_score = controller.autofocus()
    ...
    if status == 0 and valid_tiles == 16 and saw_nonzero == 1 and queue_count == 0:
        status = 3
    ...
    if status != 3:
        ...
        for candidate_index in range(7):
            slot = candidate_index + 16
            if slot < queue_count and slot < 39 - recovery_count:
                controller.mark_candidate_from_blob(slot)
