...
**Public result contract.** Declare the outcome exactly once with
`controller.complete_scan(status_code)`, then call `controller.release()` exactly
once before `run` ends. The harness constructs the machine-readable result from
that recorded outcome and checks that it agrees with controller telemetry. You do
not need to return a dictionary or repeat the status in Python; any optional
return value is ignored. The harness does not choose or correct the outcome for
you. Missing completion, runtime errors, invalid evidence, incorrect stopping,
and failure to release remain failures. Development and private evaluation use
this same interface; private cases remain unavailable during preparation.
...
**Partial results.** A legitimate chip-state or focus abort must still report candidates accumulated from valid captures before scanning stopped. Register those queued candidates before `complete_scan`, keep the appropriate abort status, and then release the instrument. These are partial simulator results, not a completed search or evidence that physical measurements remain valid after a chip change. An illumination abort still requires an empty candidate manifest.
